...
// importing dispatch hook from redux
import { useDispatch } from "react-redux";

// importing actions from contact Reducer
import { setShowContact } from "../Redux/Reducers/contactReducer";


// adding a single list item to the contact list
const ListItem = (props) => {

    // adding a dispatch variable for calling function
    const dispatch = useDispatch();

    // getting values of name and contact from the props
    const {name, phone} = props.contact;

    // handles user click on a contact item
    const handleClick = (e) =>{
        // setting the contact value inside the showContact variable and show the contact's more info
