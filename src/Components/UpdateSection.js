...
// importing the toaster notifications
import { toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

// importing functions from contact reducer
import { contactSelector, deleteContactThunk, 
        setShowContact, updateContactThunk } from "../Redux/Reducers/contactReducer";



// showing more info about a contact and update the data
const UpdateSection = () => {
    // dispatching an action
    const dispatch = useDispatch();

    // whether to show or hide the following section
    const {showContact} = useSelector(contactSelector);

    //storing the updated values of user
    const [formData, setFormData] = useState({});
    //storing the updated address
    const [address,setAddress] = useState({});

    // to check if user made some changes inside the data
    const [isChanged,setIsChanged]=useState(false);

    // initializing the values inside the above variables on page render
    useEffect(()=>{
        // setting the data
    ...
    // handling the changes made inside the input bars
    const handleChange = (e) => {

        // setting isChanged to true so that user can submit the data
        if(!isChanged){
            setIsChanged(true);
        }
        ...
        // storing the values inside the object at their respective positions
        setFormData({ 
            ...formData,
            [name]:value,
        })
    ...
    // handling the changes inside the address input bar
    const handleAddressChange = (e) => {

        // setting change to true so that user can submit the data
        if(!isChanged){
            setIsChanged(true);
        }

        // getting the values of targeted element
        const {name,value} = e.target;
        
        // storing the values of address
        setAddress({
            ...address,
            [name]:value,
        })

        // storing the address inside the main object
        setFormData({
            ...formData,
            address:{
                ...address,
    ...
    const handleSubmit = (e) => {
        ...
        if(!isChanged){
            // showing notification and returning
            toast.error('Nothing to update in contact !!');
            return;
        }

        //checking whether the contact contains a name and phone number
        ...
        e.preventDefault();
        // calling the function for updating the contact data
    ...
    // handling the contact delete
    const handleDelete = (e) => {
        e.preventDefault();
        // calling the function for deleting the contact
    ...
    // rendering the update contact section
    return(
        <>
